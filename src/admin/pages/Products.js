...
const Products = () => {
  ...
  const removeGallery = (index) => {
    setForm((prev) => {
      ...
      return { ...prev, image_gallery: gallery.length ? gallery : [""] };
    });
  };
  ...
  const handleCreate = async (e) => {
    ...
    setSubmitting(true);
    try {
      await addDoc(collection(db, "products"), buildPayload());
      resetForm();
      await fetchProducts();
    } catch (e) {
      console.error(e);
  ...
  const handleUpdate = async (e) => {
    ...
    setSubmitting(true);
    try {
      await updateDoc(doc(db, "products", editingId), buildPayload());
      resetForm();
      await fetchProducts();
    } catch (e) {
      console.error(e);
              ...
              <form onSubmit={mode === "edit" ? handleUpdate : handleCreate}>
                ...
                <div className="mb-4">
                  ...
                  {form.image_gallery.map((u, i) => (
                    <div className="input-group mb-2" key={`img-${i}`}>
                      <input
                        type="url"
                        className="form-control"
                        value={u}
